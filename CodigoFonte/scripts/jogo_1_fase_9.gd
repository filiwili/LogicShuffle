...
func verificar_primeira_conclusao():
	...
	if ProgressManager:
		var level_data = ProgressManager.get_level_data("nivel9", "1")
...
func verificar_solucao() -> Dictionary:
	# Verificar o deque (Galeria Dupla): H, H, H, C, H
	var ordem_deque_esperada = ["H", "H", "H", "C", "H"]
	var ordem_fila_esperada = ["E", "C"]
			...
			for i in range(deque.size()):
				if deque[i] != ordem_deque_esperada[i]:
					...
					match i:
						...
						3: posicao = "quarta posição"
						4: posicao = "quinta posição"
	...
	if erros.size() == 0:
		return {"is_valid": true, "message": "🎉 Parabéns! José dominou mais uma bruxaria avançada! Ambas as estruturas estão com as ordens corretas!"}
	...
	mensagem_erro += "\nLembre-se:\n"
	mensagem_erro += "- Galeria Dupla (deque): Diamante, Diamante, Diamante, Cogumelo, Diamante\n"
	mensagem_erro += "- Túnel Linear (fila): José, Cogumelo"
...
func mostrar_solucao():
	...
	# Preencher o deque (Galeria Dupla) com: H, H, H, C, H
	deque.append("H")  # Primeiro Diamante
	deque.append("H")  # Segundo Diamante
	deque.append("H")  # Terceiro Diamante
	deque.append("C")  # Cogumelo
	deque.append("H")  # Quarto Diamante
	
	# Preencher a fila (Túnel Linear) com: E, C
	fila.append("E")  # José
	fila.append("C")  # Cogumelo
...
func salvar_resultado():
	...
	if primeira_conclusao:
		...
		if ProgressManager:
			...
			ProgressManager.mark_level_completed("nivel9", pontos)
...
José continuará suas bruxarias avançadas, agora ele utilizará a Galeria Dupla e o Túnel Linear:
Em Galeria Dupla, você precisará de: Diamante, Diamante, Diamante, Cogumelo, Diamante
Em Túnel Linear, você precisará de: José, Cogumelo
""" 
...
func _on_btn_fechar():
	...
	save.set_value("nivel9", "historia_vista", true)
