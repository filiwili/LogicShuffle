...
func verificar_primeira_conclusao():
	...
	if ProgressManager:
		var level_data = ProgressManager.get_level_data("nivel4", "1")
...
func verificar_solucao() -> Dictionary:
	...
	var estruturas_nao_vazias = []
	
	if fila.size() > 0:
		estruturas_nao_vazias.append("Túnel Linear (fila) com %d elemento(s)" % fila.size())
	
	if pilha.size() > 0:
		estruturas_nao_vazias.append("Túnel Invertido (pilha) com %d elemento(s)" % pilha.size())
	
	if deque.size() > 0:
		estruturas_nao_vazias.append("Galeria Dupla (deque) com %d elemento(s)" % deque.size())
	
	# Se todas estão vazias, sucesso
	if estruturas_nao_vazias.size() == 0:
		return {"is_valid": true, "message": "🎉 Parabéns! Todas as estruturas estão vazias! José pode tirar sua soneca tranquilamente, sabendo que todos os seus bens estão seguros!"}
	
	# Construir mensagem de erro detalhada
	var mensagem_erro = "Ainda existem elementos em algumas estruturas:\n\n"
	
	for i in range(estruturas_nao_vazias.size()):
		mensagem_erro += "• " + estruturas_nao_vazias[i] + "\n"
	
	mensagem_erro += "\nUse os botões de remoção (REMOVER) para esvaziar todas as estruturas."
	
	# Dica específica baseada na estrutura com mais elementos
	var estrutura_mais_cheia = ""
	var max_elementos = 0
	
	if fila.size() > max_elementos:
		max_elementos = fila.size()
		estrutura_mais_cheia = "Túnel Linear"
	
	if pilha.size() > max_elementos:
		max_elementos = pilha.size()
		estrutura_mais_cheia = "Túnel Invertido"
	
	if deque.size() > max_elementos:
		max_elementos = deque.size()
		estrutura_mais_cheia = "Galeria Dupla"
	
	if max_elementos > 0:
		mensagem_erro += "\n\nDica: Comece pelo " + estrutura_mais_cheia + " que tem " + str(max_elementos) + " elemento(s)."
...
func mostrar_solucao():
	# Solução: simplesmente esvaziar todas as estruturas
	fila.clear()
	pilha.clear()
	deque.clear()
...
func salvar_resultado():
	...
	if primeira_conclusao:
		...
		if ProgressManager:
			...
			ProgressManager.mark_level_completed("nivel4", pontos)
...
José, agora, está planejando tirar uma soneca, mas gostaria de coletar todos os seus bens até o momento.
Para tranquilizar josé, tire todos os elementos de todas as estruturas.
""" 
...
func _on_btn_fechar():
	...
	save.set_value("nivel4", "historia_vista", true)
