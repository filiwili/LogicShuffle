...
func verificar_primeira_conclusao():
	...
	if ProgressManager:
		var level_data = ProgressManager.get_level_data("nivel1", "1")
...
func verificar_solucao() -> Dictionary:
	var e_count = 0
	if fila.has("E"): e_count += 1
	if pilha.has("E"): e_count += 1
	if deque.has("E"): e_count += 1

	if e_count != 1:
		return {"is_valid": false, "message": "O José deve estar em apenas uma caminho! Atualmente ele está em " + str(e_count) + " caminhos."}
	
	if not pilha.has("E"):
		return {"is_valid": false, "message": "O José deve estar no túnel invertida para alcançar seu objetivo!"}
	
	if pilha.size() == 0 or pilha[0] != "E":
		return {"is_valid": false, "message": "O José (E) deve ser o primeiro do túnel invertido!"}
	
	return {"is_valid": true, "message": "🎉 Parabéns! O José descobriu novos horizontes!"}
...
func mostrar_solucao():
	...
	# Configurar a solução correta: José (E) sozinho na pilha
	pilha.append("E")
...
func salvar_resultado():
	...
	if primeira_conclusao:
		...
		if ProgressManager:
			...
			ProgressManager.mark_level_completed("nivel1", pontos)
...
José está preso no [b]Túnel Linear[/b], mas descobriu uma saída secreta
no [b]Túnel Invertido[/b] que ninguém está usando.
Ajude o José a descobrir novos horizontes.
""" 
...
func _on_btn_fechar():
	...
	save.set_value("nivel1", "historia_vista", true)
