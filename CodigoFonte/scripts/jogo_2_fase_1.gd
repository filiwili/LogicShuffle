...
func configurar_texto_tutorial():
	...
	Ó jovem sacerdote, Osíris, o deus da ordem e do renascimento, te convoca! Construa a primeira Árvore da Vida com três placas sagradas. A raiz deve ser o 2, caso a placa seja maior, conecte à direita, caso contrário, à esquerda. Cada placa deve ter somente até duas ligaçãos diretas! Esta é a base de toda sabedoria!
	
	Mandamento de Osíris: Ordem de nascimento das placas: 2, 1, 3 - Siga a sabedoria de Osíris: a primeira placa é a raiz (2), depois conecte à esquerda os menores ou iguais (1) e à direita os maiores (3)."
	...
	Para este nível, assim como a interpretação do texto acima, você deve construir a seguinte árvore, utilizando as placas de acordo:

	   2
	  / \\
	 1   3

	Dica: Comece pela placa raiz (2) e depois adicione os filhos.
	"""
...
func verificar_primeira_conclusao():
	...
	if err == OK and save.has_section_key("arvore_binaria_nivel1", "concluido"):
...
func connect_nodes(parent_id: int, child_id: int):
	...
	# CORREÇÃO: números iguais agora vão para a esquerda (<= em vez de <)
	var is_left_child = child_value <= parent_value  # MUDANÇA AQUI: < para <=
...
func _on_submit_pressed():
	...
	var validation_result = validate_tree_by_traversal(tree_data)
	
	# Sempre mostrar popup com detalhes do erro/sucesso
...
func _on_confirmar_mostrar_solucao():
	...
	# ATUALIZAR A PONTUAÇÃO VISUAL IMEDIATAMENTE
	atualizar_ui_tempo()
	...
	info_popup.title = "Solução Aplicada"
	info_popup.dialog_text = "Solução aplicada! Sua pontuação foi registrada como 500 pontos."
...
func mostrar_solucao():
	print("🎯 Aplicando solução do nível 1...")
	
	# Limpar a árvore atual
	clear_tree()
	
	# Aguardar um frame para garantir que a árvore foi limpa
	await get_tree().process_frame
	
	# Adicionar nós nas posições corretas
	var raiz_pos = Vector2(400, 100)
	var esquerda_pos = Vector2(250, 200)
	var direita_pos = Vector2(550, 200)
	
	# Adicionar nós
	add_node_solucao(raiz_pos, "2")
	add_node_solucao(esquerda_pos, "1")
	add_node_solucao(direita_pos, "3")
	
	# Aguardar mais um frame para os nós serem criados
	await get_tree().process_frame
	
	# Conectar os nós (raiz -> esquerda, raiz -> direita)
	if nodes.size() >= 3:
		# Encontrar os IDs dos nós (eles serão 1, 2, 3 já que começamos do zero)
		var raiz_id = 1
		var esquerda_id = 2
		var direita_id = 3
		
		# Conectar raiz com filho esquerdo
		if can_connect_as_parent(raiz_id, esquerda_id):
			connect_nodes(raiz_id, esquerda_id)
		
		# Conectar raiz com filho direito
		if can_connect_as_parent(raiz_id, direita_id):
			connect_nodes(raiz_id, direita_id)
		
		print("✅ Solução aplicada: Árvore 2-1-3")
		
		if mensagem_label:
			mensagem_label.text = "Solução aplicada! Árvore: 2 (raiz), 1 (esquerda), 3 (direita)"
	else:
...
func validate_tree_by_traversal(tree_data: Dictionary) -> Dictionary:
	...
	if root_id == -1:
		return {"is_valid": false, "message": "Árvore vazia! Adicione pelo menos uma placa."}
	...
	if connected_nodes.size() < tree_data.nodes.size():
		...
		return {"is_valid": false, "message": "Árvore desconexa! Existem " + str(disconnected_count) + " placas isolados que não estão conectados à raiz."}
	
	return validate_level_1_exact()

func validate_level_1_exact() -> Dictionary:
	if nodes.size() != 3:
		return {"is_valid": false, "message": "O nível 1 requer exatamente 3 placas! Você tem " + str(nodes.size()) + " placas."}
	
	var root_id = find_root_node()
	if root_id == -1:
		return {"is_valid": false, "message": "Nenhuma raiz encontrada!"}
	...
	if root_value != "2":
		return {"is_valid": false, "message": "A raiz deve ter valor 2! Sua raiz tem valor " + root_value + "."}
	
	if left_child_id == -1 or right_child_id == -1:
		return {"is_valid": false, "message": "A raiz deve ter dois filhos!"}
	
	var left_value = nodes[left_child_id].value
	var right_value = nodes[right_child_id].value
	
	if left_value != "1" or right_value != "3":
		return {"is_valid": false, "message": "Os filhos devem ter valores 1 (esquerda) e 3 (direita)! Seus filhos têm valores " + left_value + " e " + right_value + "."}
	
	var preorder = get_preorder_traversal(root_id)
	var expected_preorder = ["2", "1", "3"]
	
	if preorder != expected_preorder:
		return {"is_valid": false, "message": "Estrutura incorreta! A árvore deve ser: 2 como raiz, 1 à esquerda, 3 à direita."}
	
	return {"is_valid": true, "message": "✅ Árvore perfeita! Estrutura exata do nível 1 construída corretamente."}
...
func salvar_resultado():
	...
	save.set_value("arvore_binaria_nivel1", "concluido", true)
	save.set_value("arvore_binaria_nivel1", "pontuacao", pontuacao)
	save.set_value("arvore_binaria_nivel1", "tempo", tempo_decorrido)
	...
	if ProgressManager:
		...
		ProgressManager.mark_level_completed("arvore_binaria_nivel1", pontuacao)
