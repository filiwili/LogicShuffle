...
# Variável para controlar a ordem de conexão
var connection_order: Array = []

func _ready():
	print("=== INICIANDO JOGO 2 FASE 2 (THOTH) ===")
...
func configurar_texto_tutorial():
	...
	[b]Nível 7: A SABEDORIA DE THOTH II [/b]
	...
	Ó jovem sacerdote, Thoth, o deus da sabedoria e do equilíbrio, te convoca! Siga meu caminho harmonioso: primeiro a esquerda, depois a raiz e então a direita. Como a balança da justiça que pende para os lados antes de encontrar o equilíbrio central, organize os valores na sequência que eu ordenar!
	
	Neste Nível, as conexões à raiz serão primeiramente feitas para a esquerda, e então, para a direita
	[b]Mandamento de Thoth:[/b]
	Ordem de nascimento das placas: 7, 1, 3
	Leia como: [b]ESQUERDA-RAIZ-DIREITA[/b]
	
	• Use a ferramenta [b]Adicionar Placa[/b] para construir as placas na tela.
	• Use a ferramenta [b]Conectar Placas[/b] para ligar as placas conforme a sabedoria de Thoth.
...
func verificar_primeira_conclusao():
	...
	if err == OK and save.has_section_key("arvore_binaria_nivel7", "concluido"):
...
func connect_nodes(parent_id: int, child_id: int):
	...
	# REGRA MODIFICADA PARA O NÍVEL 3: Primeira conexão vai para esquerda, segunda para direita
...
func _on_submit_pressed():
	...
	var validation_result = validate_level_7_thoth()  # Mude para validate_level_7_thoth
...
func _on_confirmar_mostrar_solucao():
	...
	info_popup.title = "Sabedoria de Thoth Revelada"
...
func mostrar_solucao():
	print("🎯 Aplicando solução do nível 7 (Thoth)...")
	...
	# CORREÇÃO: Adicionar nós na ordem correta: 1 (raiz), 7 (esquerda), 3 (direita)
	add_node_solucao(raiz_pos, "1")
	add_node_solucao(esquerda_pos, "7")
	add_node_solucao(direita_pos, "3")
	...
	if nodes.size() >= 3:
		...
		if nodes[raiz_id].value == "1" and nodes[esquerda_id].value == "7" and nodes[direita_id].value == "3":
			# Conectar raiz (1) com filho esquerdo (7)
			if can_connect_as_parent(raiz_id, esquerda_id):
				connect_nodes(raiz_id, esquerda_id)
				await get_tree().process_frame
				
				# Conectar raiz (1) com filho direito (3)
				if can_connect_as_parent(raiz_id, direita_id):
					connect_nodes(raiz_id, direita_id)
					print("✅ Solução Thoth aplicada: Árvore 1-7-3")
					
					if mensagem_label:
						mensagem_label.text = "Solução aplicada! Árvore de Thoth: 1 (raiz), 7 (esquerda), 3 (direita)"
				else:
...
func validate_level_7_thoth() -> Dictionary:
	if nodes.size() != 3:
		return {"is_valid": false, "message": "Thoth exige exatamente 3 placas sagradas! Você tem " + str(nodes.size()) + " placas."}
	
	var root_id = find_root_node()
	if root_id == -1:
		return {"is_valid": false, "message": "Nenhuma raiz encontrada! Thoth demanda uma árvore com raiz."}
	
	var root_value = nodes[root_id].value
	var left_child_id = nodes[root_id].left_child
	var right_child_id = nodes[root_id].right_child
	
	# CORREÇÃO: Raiz deve ser 1 (conforme a ordem ESQUERDA-RAIZ-DIREITA)
	if root_value != "1":
		return {"is_valid": false, "message": "Thoth ordena que a raiz seja 1! Sua raiz é " + root_value + "."}
	
	if left_child_id == -1 or right_child_id == -1:
		return {"is_valid": false, "message": "Thoth exige dois filhos para a raiz! A balança precisa de equilíbrio."}
	
	var left_value = nodes[left_child_id].value
	var right_value = nodes[right_child_id].value
	
	# CORREÇÃO: Filhos devem ser 7 (esquerda) e 3 (direita)
	if left_value != "7" or right_value != "3":
		return {"is_valid": false, "message": "Thoth demanda filhos 7 (esquerda) e 3 (direita)! Seus filhos são " + left_value + " e " + right_value + "."}
	
	var inorder = get_inorder_traversal(root_id)
	var expected_inorder = ["7", "1", "3"]  # CORREÇÃO: In-ordem deve ser 7, 1, 3
	
	if inorder != expected_inorder:
		return {"is_valid": false, "message": "A ordem de Thoth não foi seguida! Em-ordem deve ser: 7, 1, 3. Sua ordem: " + str(inorder)}
	
	return {"is_valid": true, "message": "✅ Sabedoria de Thoth alcançada! A árvore está perfeitamente equilibrada."}
...
func salvar_resultado():
	...
	save.set_value("arvore_binaria_nivel7", "concluido", true)
	save.set_value("arvore_binaria_nivel7", "pontuacao", pontuacao)
	save.set_value("arvore_binaria_nivel7", "tempo", tempo_decorrido)
	...
	if ProgressManager:
		...
		ProgressManager.mark_level_completed("arvore_binaria_nivel7", pontuacao)
...
func validate_tree_by_traversal(tree_data: Dictionary) -> Dictionary:
	...
	if root_id == -1:
		return {"is_valid": false, "message": "Árvore vazia! Thoth exige uma árvore sagrada. Adicione pelo menos uma placa."}
	...
	if connected_nodes.size() < tree_data.nodes.size():
		...
		return {"is_valid": false, "message": "Árvore desconexa! Existem " + str(disconnected_count) + " placas isoladas que não estão conectadas à raiz. Thoth demanda que todas as placas estejam unidas."}
	
	return validate_level_7_thoth()  # Mude para validate_level_7_thoth
