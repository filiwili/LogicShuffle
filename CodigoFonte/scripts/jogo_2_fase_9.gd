...
func configurar_texto_tutorial():
	...
	[b]Nível 9: A ÁRVORE DA VIDA III [/b]
	...
	Ó jovem sacerdote, Osíris, o deus da ordem e do renascimento, te convoca! Construa a primeira Árvore da Vida com três placas sagradas. A raiz deve ser o 5, caso a placa seja maior, conecte à direita, caso contrário, à esquerda. Cada placa deve ter somente até duas ligaçãos diretas! Esta é a base de toda sabedoria!
	
	Mandamento de Osíris: Ordem de nascimento das placas: 5, 6, 4, 3, 7 - Siga a sabedoria de Osíris: a primeira placa é a raiz (5), depois conecte à esquerda os menores ou iguais e à direita os maiores."
...
func verificar_primeira_conclusao():
	...
	if err == OK and save.has_section_key("arvore_binaria_nivel9", "concluido"):
...
func _on_submit_pressed():
	...
	var validation_result = validate_level_9_osiris()  # Mude para validate_level_9_osiris
...
func _on_confirmar_mostrar_solucao():
	...
	# Mudar para solução do nível 9
	mostrar_solucao_nivel9()  # Mude para mostrar_solucao_nivel9
...
func mostrar_solucao_nivel9():
	print("🎯 Aplicando solução do nível 9 (Osíris - BST com 5 nós)...")
	...
	# Posições para a árvore BST 5, 6, 4, 3, 7
	var raiz_pos = Vector2(screen_center.x - 40, 100)        # 5 (raiz)
	var direita_pos = Vector2(screen_center.x + 100, 200)   # 6 (direita de 5)
	var esquerda_pos = Vector2(screen_center.x - 150, 200)  # 4 (esquerda de 5)
	var esquerda_esquerda_pos = Vector2(screen_center.x - 200, 300)  # 3 (esquerda de 4)
	var direita_direita_pos = Vector2(screen_center.x + 150, 300)    # 7 (direita de 6)
	
	# Adicionar nós na ordem correta: 5 (raiz), 4 (esquerda), 3 (esquerda de 4), 6 (direita), 7 (direita de 6)
	add_node_solucao(raiz_pos, "5")
	add_node_solucao(esquerda_pos, "4")
	add_node_solucao(direita_pos, "6")
	add_node_solucao(esquerda_esquerda_pos, "3")
	...
	if nodes.size() >= 5:
		...
		if (nodes[node_ids[0]].value == "5" and nodes[node_ids[1]].value == "4" and 
			nodes[node_ids[2]].value == "6" and nodes[node_ids[3]].value == "3" and 
			nodes[node_ids[4]].value == "7"):
			
			var raiz_id = node_ids[0]  # 5
			var esquerda_id = node_ids[1]  # 4
			var direita_id = node_ids[2]  # 6
			var esquerda_esquerda_id = node_ids[3]  # 3
			var direita_direita_id = node_ids[4]  # 7
			
			# Conectar raiz (5) com filho esquerdo (4)
			if can_connect_as_parent(raiz_id, esquerda_id):
				connect_nodes(raiz_id, esquerda_id)
				await get_tree().process_frame
			
			# Conectar raiz (5) com filho direito (6)
			if can_connect_as_parent(raiz_id, direita_id):
				connect_nodes(raiz_id, direita_id)
				await get_tree().process_frame
			
			# Conectar nó 4 com filho esquerdo (3)
			if can_connect_as_parent(esquerda_id, esquerda_esquerda_id):
				connect_nodes(esquerda_id, esquerda_esquerda_id)
				await get_tree().process_frame
			
			# Conectar nó 6 com filho direito (7)
			if can_connect_as_parent(direita_id, direita_direita_id):
				connect_nodes(direita_id, direita_direita_id)
				print("✅ Solução Osíris aplicada: BST 5-4-6-3-7")
				
				if mensagem_label:
					mensagem_label.text = "Solução aplicada!s"
			else:
				print("❌ Erro ao conectar 6 com 7")
		else:
...
func validate_tree_by_traversal(tree_data: Dictionary) -> Dictionary:
	...
	# Para o nível 9, validar a BST específica com 5 nós
	return validate_level_9_osiris()  # Mude para validate_level_9_osiris

func validate_level_9_osiris() -> Dictionary:
	# Verificar número de nós - agora são 5 nós
	if nodes.size() != 5:
		return {"is_valid": false, "message": "Osíris exige exatamente 5 placas sagradas! Você tem " + str(nodes.size()) + " placas."}
	...
	if root_value != "5":
		return {"is_valid": false, "message": "Osíris ordena que a raiz seja 5! Sua raiz é " + root_value + "."}
	...
	# Verificar estrutura BST específica para 5, 6, 4, 3, 7
	# Estrutura esperada:
	#       5
	#      / \
	#     4   6
	...
	# Verificar filho esquerdo da raiz (deve ser 4)
	if root_node.left_child == -1:
		return {"is_valid": false, "message": "A raiz 5 deve ter um filho à esquerda (4)!"}
	var left_child_value = nodes[root_node.left_child].value
	if left_child_value != "4":
		return {"is_valid": false, "message": "O filho esquerdo da raiz deve ser 4! Seu filho esquerdo é " + left_child_value + "."}
	
	# Verificar filho direito da raiz (deve ser 6)
	if root_node.right_child == -1:
		return {"is_valid": false, "message": "A raiz 5 deve ter um filho à direita (6)!"}
	var right_child_value = nodes[root_node.right_child].value
	if right_child_value != "6":
		return {"is_valid": false, "message": "O filho direito da raiz deve ser 6! Seu filho direito é " + right_child_value + "."}
	
	# Verificar filho esquerdo do nó 4 (deve ser 3)
	var left_child_node = nodes[root_node.left_child]
	if left_child_node.left_child == -1:
		return {"is_valid": false, "message": "O nó 4 deve ter um filho à esquerda (3)!"}
	var left_left_value = nodes[left_child_node.left_child].value
	if left_left_value != "3":
		return {"is_valid": false, "message": "O filho esquerdo do nó 4 deve ser 3! Seu filho esquerdo é " + left_left_value + "."}
	
	# Verificar filho direito do nó 6 (deve ser 7)
	var right_child_node = nodes[root_node.right_child]
	if right_child_node.right_child == -1:
		return {"is_valid": false, "message": "O nó 6 deve ter um filho à direita (7)!"}
	var right_right_value = nodes[right_child_node.right_child].value
	if right_right_value != "7":
		return {"is_valid": false, "message": "O filho direito do nó 6 deve ser 7! Seu filho direito é " + right_right_value + "."}
	
	# Verificar travessia in-order
	var inorder = get_inorder_traversal(root_id)
	var expected_inorder = ["3", "4", "5", "6", "7"]
	...
	return {"is_valid": true, "message": "✅ Sabedoria de Osíris alcançada! A BST de 5 nós está perfeitamente ordenada."}
...
func salvar_resultado():
	...
	save.set_value("arvore_binaria_nivel9", "concluido", true)
	save.set_value("arvore_binaria_nivel9", "pontuacao", pontuacao)
	save.set_value("arvore_binaria_nivel9", "tempo", tempo_decorrido)
	...
	if ProgressManager:
		...
		ProgressManager.mark_level_completed("arvore_binaria_nivel9", pontuacao)
