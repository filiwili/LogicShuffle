...
func verificar_primeira_conclusao():
	...
	if ProgressManager:
		var level_data = ProgressManager.get_level_data("nivel5", "1")
...
func verificar_solucao() -> Dictionary:
	# A ordem esperada na fila: Pedra (A), Cristal (B), José (E), Moeda (F), Pérola (G)
	var ordem_esperada = ["A", "B", "E", "F", "G"]
	
	# Verificar se a fila está exatamente com a ordem esperada
	if fila == ordem_esperada:
		return {"is_valid": true, "message": "🎉 Parabéns! A bruxaria de José foi realizada com sucesso! A ordem no Túnel Linear está correta: Pedra, Cristal, José, Moeda, Pérola."}
	
	# Verificar se a fila tem o tamanho correto
	if fila.size() != ordem_esperada.size():
		return {"is_valid": false, "message": "A fila não tem a quantidade correta de elementos. Esperados: %d, Atuais: %d." % [ordem_esperada.size(), fila.size()]}
	
	# Verificar elemento por elemento
	for i in range(fila.size()):
		if fila[i] != ordem_esperada[i]:
			...
			return {"is_valid": false, "message": "Ordem incorreta na posição %d. Esperado: %s (%s), Atual: %s (%s)." % [i+1, ordem_esperada[i], elemento_esperado, fila[i], elemento_atual]}
	
	# Caso genérico de erro
	return {"is_valid": false, "message": "A ordem no Túnel Linear não está correta. A ordem esperada é: Pedra (A), Cristal (B), José (E), Moeda (F), Pérola (G)."}
...
func mostrar_solucao():
	...
	# Preencher a fila com a ordem correta
	fila.append_array(["A", "B", "E", "F", "G"])
...
func salvar_resultado():
	...
	if primeira_conclusao:
		...
		if ProgressManager:
			...
			ProgressManager.mark_level_completed("nivel5", pontos)
...
José agora começará suas bruxarias, a fim de fazer bom uso da caverna.
A bruxaria de José requer que seja feita no Túnel Linear, com a ordem do primeiro ao último elemento, sendo: Pedra, Cristal, José, Moeda, Pérola
""" 
...
func _on_btn_fechar():
	...
	save.set_value("nivel5", "historia_vista", true)
