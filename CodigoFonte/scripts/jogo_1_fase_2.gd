...
func verificar_primeira_conclusao():
	...
	if ProgressManager:
		var level_data = ProgressManager.get_level_data("nivel2", "1")
...
func verificar_solucao() -> Dictionary:
	# Verificar se a moeda (F) está no topo da pilha
	if pilha.size() > 0 and pilha[0] == "F":
		return {"is_valid": true, "message": "🎉 Parabéns! A moeda está no topo do Túnel Invertido! José pode comprar seus materiais!"}
	
	# Verificar se a moeda está em outra posição na pilha
	if pilha.has("F"):
		return {"is_valid": false, "message": "A moeda (F) está no Túnel Invertido, mas não está no topo! Use as operações para movê-la para a primeira posição."}
	
	# Verificar se a moeda está em outras estruturas
	if fila.has("F"):
		return {"is_valid": false, "message": "A moeda (F) está no Túnel Linear. Use o botão de redirecionamento para movê-la para o Túnel Invertido."}
	
	if deque.has("F"):
		return {"is_valid": false, "message": "A moeda (F) está na Galeria Dupla. Use os botões de redirecionamento para movê-la para o Túnel Invertido."}
	
	return {"is_valid": false, "message": "A moeda (F) não foi encontrada em nenhum caminho. Você precisa adicioná-la usando o menu de seleção de elementos."}
...
func salvar_resultado():
	...
	if primeira_conclusao:
		...
		if ProgressManager:
			...
			ProgressManager.mark_level_completed("nivel2", pontos)
...
Após José descobrir a saída secreta da fase anterior, ele se depara com um vendedor ambulante e se interessou em comprar alguns materiais, para isso ser possível, a moeda tem que estar no topo do Túnel Invertido (T.I)
Ajude o José a atingir sua meta.

Dica: utilize os botões de redirecionamento dos outros caminhos mágicos, de modo que a moeda fique no topo.
""" 
...
func _on_btn_fechar():
	...
	save.set_value("nivel2", "historia_vista", true)
