...
func verificar_primeira_conclusao():
	...
	if ProgressManager:
		var level_data = ProgressManager.get_level_data("nivel6", "1")
...
func verificar_solucao() -> Dictionary:
	# A ordem esperada no deque: Moeda (F), Pérola (G), Diamante (H), Cogumelo (C), Osso (D)
	var ordem_esperada = ["F", "G", "H", "C", "D"]
	
	# Verificar se o deque está exatamente com a ordem esperada
	if deque == ordem_esperada:
		return {"is_valid": true, "message": "🎉 Parabéns! A bruxaria na Galeria Dupla foi realizada com sucesso! A ordem está correta: Moeda, Pérola, Diamante, Cogumelo, Osso."}
	
	# Verificar se o deque tem o tamanho correto
	if deque.size() != ordem_esperada.size():
		return {"is_valid": false, "message": "O deque não tem a quantidade correta de elementos. Esperados: %d, Atuais: %d." % [ordem_esperada.size(), deque.size()]}
	
	# Verificar elemento por elemento
	for i in range(deque.size()):
		if deque[i] != ordem_esperada[i]:
			...
			match i:
				...
				3: posicao = "quarta"
				4: posicao = "quinta"
			return {"is_valid": false, "message": "Ordem incorreta na %s posição. Esperado: %s (%s), Atual: %s (%s)." % [posicao, ordem_esperada[i], elemento_esperado, deque[i], elemento_atual]}
	
	# Caso genérico de erro
	return {"is_valid": false, "message": "A ordem na Galeria Dupla não está correta. A ordem esperada é: Moeda (F), Pérola (G), Diamante (H), Cogumelo (C), Osso (D)."}
...
func mostrar_solucao():
	...
	# Preencher o deque com a ordem correta
	deque.append_array(["F", "G", "H", "C", "D"])
...
func salvar_resultado():
	...
	if primeira_conclusao:
		...
		if ProgressManager:
			...
			ProgressManager.mark_level_completed("nivel6", pontos)
...
José está preparando mais uma bruxaria: dessa vez na Galeria Dupla
Organize os elementos na ordem de: Moeda, Pérola, Diamante, Cogumelo, Osso para conclur a bruxaria da vez.
""" 
...
func _on_btn_fechar():
	...
	save.set_value("nivel6", "historia_vista", true)
