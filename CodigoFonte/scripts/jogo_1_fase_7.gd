...
func verificar_primeira_conclusao():
	...
	if ProgressManager:
		var level_data = ProgressManager.get_level_data("nivel7", "1")
...
func verificar_solucao() -> Dictionary:
	# A ordem esperada na pilha (do topo para o fundo): Pedra (A), Moeda (F), Diamante (H), Cogumelo (C)
	var ordem_esperada = ["A", "F", "H", "C"]
	
	# Verificar se a pilha está exatamente com a ordem esperada
	if pilha == ordem_esperada:
		return {"is_valid": true, "message": "🎉 Parabéns! A bruxaria no Túnel Invertido foi um sucesso! A ordem do topo para o fundo está correta: Pedra, Moeda, Diamante, Cogumelo."}
	
	# Verificar se a pilha tem o tamanho correto
	if pilha.size() != ordem_esperada.size():
		return {"is_valid": false, "message": "A pilha não tem a quantidade correta de elementos. Esperados: %d, Atuais: %d." % [ordem_esperada.size(), pilha.size()]}
	
	# Verificar elemento por elemento (do topo para o fundo)
	for i in range(pilha.size()):
		if pilha[i] != ordem_esperada[i]:
			var elemento_esperado = elementos_caverna[ordem_esperada[i]]["nome"]
			var elemento_atual = elementos_caverna[pilha[i]]["nome"] if elementos_caverna.has(pilha[i]) else "Desconhecido"
			var posicao = ""
			match i:
				0: posicao = "topo (primeira posição)"
				1: posicao = "segunda posição"
				2: posicao = "terceira posição"
				3: posicao = "fundo (quarta posição)"
			return {"is_valid": false, "message": "Ordem incorreta na %s. Esperado: %s (%s), Atual: %s (%s)." % [posicao, ordem_esperada[i], elemento_esperado, pilha[i], elemento_atual]}
	
	# Caso genérico de erro
	return {"is_valid": false, "message": "A ordem no Túnel Invertido não está correta. A ordem esperada (do topo para o fundo) é: Pedra (A), Moeda (F), Diamante (H), Cogumelo (C)."}
...
func mostrar_solucao():
	...
	# CORREÇÃO: Para obter a ordem A, F, H, C (do topo para o fundo) na pilha:
	# Devemos inserir na ordem: A, F, H, C
	# Porque na nossa implementação, a pilha é representada como um array onde:
	# - índice 0 é o TOPO (primeiro a sair)
	# - último índice é o FUNDO (último a sair)
	pilha.append("A")  # Topo - primeiro a sair
	pilha.append("F")  # Segundo
	pilha.append("H")  # Terceiro
	pilha.append("C")  # Fundo - último a sair
...
func salvar_resultado():
	...
	if primeira_conclusao:
		...
		if ProgressManager:
			...
			ProgressManager.mark_level_completed("nivel7", pontos)
...
José agora planeja uma bruxaria no Túnel Invertido:
Dessa vez ele quer ter, do topo para o fundo: uma pedra, uma moeda, um diamante e um cógumelo
""" 
...
func _on_btn_fechar():
	...
	save.set_value("nivel7", "historia_vista", true)
