...
func configurar_texto_tutorial():
	...
	[b]Nível 4: O JULGAMENTO DE ANÚBIS [/b]
	...
	Ordem de nascimento das placas: 2, 1, 3
	...
	Para este nível, você deve construir a seguinte árvore:

	   3
	  / \\
	 2   1

	[b]Dica:[/b] Comece pela placa raiz (3) e depois adicione os filhos 2 (esquerda) e 1 (direita).
	A ordem pós-ordem (ESQUERDA-DIREITA-RAIZ) deve resultar em: 2, 1, 3
	"""
...
func verificar_primeira_conclusao():
	...
	if err == OK and save.has_section_key("arvore_binaria_nivel4", "concluido"):
...
func _on_submit_pressed():
	...
	var validation_result = validate_level_4_anubis()
...
func mostrar_solucao():
	print("🎯 Aplicando solução do nível 4 (Anúbis)...")
	
	clear_tree()
	await get_tree().process_frame
	
	var raiz_pos = Vector2(400, 100)
	var esquerda_pos = Vector2(250, 200)
	var direita_pos = Vector2(550, 200)
	
	# Adicionar nós na ordem correta: 3 (raiz), 2 (esquerda), 1 (direita)
	add_node_solucao(raiz_pos, "3")
	add_node_solucao(esquerda_pos, "2")
	...
	if nodes.size() >= 3:
		...
		print("✅ Solução Anúbis aplicada: Árvore 3-2-1")
		
		if mensagem_label:
			mensagem_label.text = "Solução aplicada! Árvore de Anúbis: 3 (raiz), 2 (esquerda), 1 (direita)"
	else:
...
func validate_level_4_anubis() -> Dictionary:
	if nodes.size() != 3:
	...
	if left_value != "2" or right_value != "1":
		return {"is_valid": false, "message": "Anúbis demanda filhos 2 (esquerda) e 1 (direita)! Seus filhos são " + left_value + " e " + right_value + "."}
	
	var postorder = get_postorder_traversal(root_id)
	var expected_postorder = ["2", "1", "3"]
	
	if postorder != expected_postorder:
		return {"is_valid": false, "message": "A ordem de Anúbis não foi seguida! Pós-ordem deve ser: 2, 1, 3"}
...
func salvar_resultado():
	...
	save.set_value("arvore_binaria_nivel4", "concluido", true)
	save.set_value("arvore_binaria_nivel4", "pontuacao", pontuacao)
	save.set_value("arvore_binaria_nivel4", "tempo", tempo_decorrido)
	...
	if ProgressManager:
		...
		ProgressManager.mark_level_completed("arvore_binaria_nivel4", pontuacao)
...
func validate_tree_by_traversal(tree_data: Dictionary) -> Dictionary:
	...
	return validate_level_4_anubis()
