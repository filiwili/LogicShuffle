...
func verificar_primeira_conclusao():
	...
	if err == OK and save.has_section_key("arvore_binaria_nivel6", "concluido"):
...
func _on_submit_pressed():
	...
	var validation_result = validate_level_6_ra()  # Mude para validate_level_6_ra
...
func mostrar_solucao():
	print("🎯 Aplicando solução do nível 6 (Rá)...")
	...
	# CORREÇÃO: Adicionar nós na ordem correta: 7 (raiz), 1 (esquerda), 3 (direita)
	add_node_solucao(raiz_pos, "7")
	...
	if nodes.size() >= 3:
		...
		if nodes[raiz_id].value == "7" and nodes[esquerda_id].value == "1" and nodes[direita_id].value == "3":
			# Conectar raiz (7) com filho esquerdo (1)
			if can_connect_as_parent(raiz_id, esquerda_id):
				connect_nodes(raiz_id, esquerda_id)
				await get_tree().process_frame
				
				# Conectar raiz (7) com filho direito (3)
				if can_connect_as_parent(raiz_id, direita_id):
					connect_nodes(raiz_id, direita_id)
					print("✅ Solução Rá aplicada: Árvore 7-1-3")
					
					if mensagem_label:
						mensagem_label.text = "Solução aplicada! Árvore de Rá: 7 (raiz), 1 (esquerda), 3 (direita)"
				else:
...
func validate_level_6_ra() -> Dictionary:
	if nodes.size() != 3:
		return {"is_valid": false, "message": "Rá exige exatamente 3 placas sagradas! Você tem " + str(nodes.size()) + " placas."}
	...
	# CORREÇÃO: Raiz deve ser 7, não 2
	if root_value != "7":
		return {"is_valid": false, "message": "Rá ordena que a raiz seja 7! Sua raiz é " + root_value + "."}
	
	if left_child_id == -1 or right_child_id == -1:
		return {"is_valid": false, "message": "Rá exige dois filhos para a raiz! O sol precisa de dois caminhos."}
	
	var left_value = nodes[left_child_id].value
	var right_value = nodes[right_child_id].value
	
	# CORREÇÃO: Filhos devem ser 1 (esquerda) e 3 (direita)
	if left_value != "1" or right_value != "3":
		return {"is_valid": false, "message": "Rá demanda filhos 1 (esquerda) e 3 (direita)! Seus filhos são " + left_value + " e " + right_value + "."}
	
	var preorder = get_preorder_traversal(root_id)
	var expected_preorder = ["7", "1", "3"]  # CORREÇÃO: Pré-ordem deve ser 7, 1, 3
	
	if preorder != expected_preorder:
		return {"is_valid": false, "message": "A ordem de Rá não foi seguida! Pré-ordem deve ser: 7, 1, 3. Sua ordem: " + str(preorder)}
	
	return {"is_valid": true, "message": "✅ Glória a Rá! A árvore está perfeitamente ordenada."}
...
func salvar_resultado():
	...
	save.set_value("arvore_binaria_nivel6", "concluido", true)
	save.set_value("arvore_binaria_nivel6", "pontuacao", pontuacao)
	save.set_value("arvore_binaria_nivel6", "tempo", tempo_decorrido)
	...
	if ProgressManager:
		...
		ProgressManager.mark_level_completed("arvore_binaria_nivel6", pontuacao)
...
func validate_tree_by_traversal(tree_data: Dictionary) -> Dictionary:
	...
	return validate_level_6_ra()  # Mude para validate_level_6_ra
