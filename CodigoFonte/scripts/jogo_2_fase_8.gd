...
func _ready():
	print("=== INICIANDO JOGO 2 FASE 4 (ANÚBIS) ===")
...
func configurar_texto_tutorial():
	...
	[b]Nível 8: O JULGAMENTO DE ANÚBIS II [/b]
	...
	Ó jovem sacerdote, Anúbis, o deus do juízo final e das transformações, te convoca! Siga meu caminho ancestral: primeiro a esquerda, depois a direita e só então a raiz. Como as almas que são julgadas antes de chegarem a mim, deixe a raiz para o final - ela é o destino, não o começo!
	
	[b]Mandamento de Anúbis:[/b]
	Ordem de nascimento das placas: 7, 1, 3
	Leia como: [b]ESQUERDA-DIREITA-RAIZ[/b]
	
	• Use a ferramenta [b]Adicionar Placa[/b] para construir as placas na tela.
	• Use a ferramenta [b]Conectar Placas[/b] para ligar as placas conforme o julgamento de Anúbis.
...
func verificar_primeira_conclusao():
	...
	if err == OK and save.has_section_key("arvore_binaria_nivel8", "concluido"):
...
# FUNÇÃO CONNECT_NODES COM LÓGICA UNIVERSAL - PRIMEIRA CONEXÃO SEMPRE ESQUERDA, SEGUNDA SEMPRE DIREITA
func connect_nodes(parent_id: int, child_id: int):
	...
	# LÓGICA UNIVERSAL: Primeira conexão sempre esquerda, segunda sempre direita
...
func _on_submit_pressed():
	...
	var validation_result = validate_level_8_anubis()  # Mude para validate_level_8_anubis
...
func _on_confirmar_mostrar_solucao():
	...
	info_popup.title = "Julgamento de Anúbis Revelado"
...
func mostrar_solucao():
	print("🎯 Aplicando solução do nível 8 (Anúbis)...")
	...
	var esquerda_pos = Vector2(screen_center.x - 150, 200)
	var direita_pos = Vector2(screen_center.x + 70, 200)
	
	# CORREÇÃO: Adicionar nós na ordem correta: 3 (raiz), 7 (esquerda), 1 (direita)
	add_node_solucao(raiz_pos, "3")
	add_node_solucao(esquerda_pos, "7")
	add_node_solucao(direita_pos, "1")
	...
	if nodes.size() >= 3:
		...
		var esquerda_id = node_ids[1]  # Segundo nó
		var direita_id = node_ids[2]  # Terceiro nó
		
		# Verificar se os valores estão corretos
		if nodes[raiz_id].value == "3" and nodes[esquerda_id].value == "7" and nodes[direita_id].value == "1":
			# Conectar raiz (3) com filho esquerdo (7)
			if can_connect_as_parent(raiz_id, esquerda_id):
				connect_nodes(raiz_id, esquerda_id)
				await get_tree().process_frame
				
				# Conectar raiz (3) com filho direito (1)
				if can_connect_as_parent(raiz_id, direita_id):
					connect_nodes(raiz_id, direita_id)
					print("✅ Solução Anúbis aplicada: Árvore 3-7-1")
					
					if mensagem_label:
						mensagem_label.text = "Solução aplicada! Árvore de Anúbis: 3 (raiz), 7 (esquerda), 1 (direita)"
				else:
					print("❌ Erro ao conectar raiz com filho direito")
			else:
				print("❌ Erro ao conectar raiz com filho esquerdo")
		else:
...
func validate_level_8_anubis() -> Dictionary:
	if nodes.size() != 3:
		return {"is_valid": false, "message": "Anúbis exige exatamente 3 placas sagradas! Você tem " + str(nodes.size()) + " placas."}
	
	var root_id = find_root_node()
	if root_id == -1:
		return {"is_valid": false, "message": "Nenhuma raiz encontrada! Anúbis demanda uma árvore com raiz."}
	
	var root_value = nodes[root_id].value
	var left_child_id = nodes[root_id].left_child
	var right_child_id = nodes[root_id].right_child
	
	# CORREÇÃO: Raiz deve ser 3 (conforme a ordem ESQUERDA-DIREITA-RAIZ)
	if root_value != "3":
		return {"is_valid": false, "message": "Anúbis ordena que a raiz seja 3! Sua raiz é " + root_value + "."}
	
	if left_child_id == -1 or right_child_id == -1:
		return {"is_valid": false, "message": "Anúbis exige dois filhos para a raiz! O julgamento precisa de dois caminhos."}
	
	var left_value = nodes[left_child_id].value
	var right_value = nodes[right_child_id].value
	
	# CORREÇÃO: Filhos devem ser 7 (esquerda) e 1 (direita)
	if left_value != "7" or right_value != "1":
		return {"is_valid": false, "message": "Anúbis demanda filhos 7 (esquerda) e 1 (direita)! Seus filhos são " + left_value + " e " + right_value + "."}
	
	var postorder = get_postorder_traversal(root_id)
	var expected_postorder = ["7", "1", "3"]  # CORREÇÃO: Pós-ordem deve ser 7, 1, 3
	
	if postorder != expected_postorder:
		return {"is_valid": false, "message": "A ordem de Anúbis não foi seguida! Pós-ordem deve ser: 7, 1, 3. Sua ordem: " + str(postorder)}
	
	return {"is_valid": true, "message": "✅ Julgamento de Anúbis concluído! A árvore está perfeitamente ordenada."}

func get_postorder_traversal(node_id: int) -> Array:
	var result = []
	if node_id in nodes:
		var node = nodes[node_id]
		if node.left_child != -1:
			result.append_array(get_postorder_traversal(node.left_child))
		if node.right_child != -1:
			result.append_array(get_postorder_traversal(node.right_child))
...
func salvar_resultado():
	...
	save.set_value("arvore_binaria_nivel8", "concluido", true)
	save.set_value("arvore_binaria_nivel8", "pontuacao", pontuacao)
	save.set_value("arvore_binaria_nivel8", "tempo", tempo_decorrido)
	...
	if ProgressManager:
		...
		ProgressManager.mark_level_completed("arvore_binaria_nivel8", pontuacao)
...
func validate_tree_by_traversal(tree_data: Dictionary) -> Dictionary:
	...
	if root_id == -1:
		return {"is_valid": false, "message": "Árvore vazia! Anúbis exige uma árvore sagrada. Adicione pelo menos uma placa."}
	...
	if connected_nodes.size() < tree_data.nodes.size():
		...
		return {"is_valid": false, "message": "Árvore desconexa! Existem " + str(disconnected_count) + " placas isoladas que não estão conectadas à raiz. Anúbis demanda que todas as placas estejam unidas."}
	
	return validate_level_8_anubis()  # Mude para validate_level_8_anubis
