...
func verificar_primeira_conclusao():
	...
	if ProgressManager:
		var level_data = ProgressManager.get_level_data("nivel3", "1")
...
func verificar_solucao() -> Dictionary:
	# Verificar se há pelo menos 3 cristais no topo da pilha
	var cristais_no_topo = 0
	
	# Contar quantos cristais consecutivos estão desde o topo (início da pilha)
	for i in range(min(3, pilha.size())):
		if pilha[i] == "B":
			cristais_no_topo += 1
		else:
			break  # Se encontrar um elemento que não é cristal, para de contar
	
	# Verificação principal
	if cristais_no_topo >= 3:
		return {"is_valid": true, "message": "🎉 Parabéns! José conseguiu 3 Cristais no topo do Túnel Invertido! Agora ele pode fazer ótimos negócios!"}
	
	# Mensagens de erro específicas
	if cristais_no_topo == 2:
		return {"is_valid": false, "message": "Você tem 2 Cristais no topo, mas precisa de 3! Continue organizando."}
	
	if cristais_no_topo == 1:
		return {"is_valid": false, "message": "Apenas 1 Cristal no topo. Você precisa de mais 2 Cristais consecutivos no topo da pilha."}
	
	# Verificar se existem cristais na pilha, mas não no topo
	if pilha.has("B"):
		var posicao = pilha.find("B")
		return {"is_valid": false, "message": "Existem Cristais na pilha, mas não estão no topo. O primeiro Cristal está na posição %d (topo é 0)." % posicao}
	
	# Verificar cristais em outras estruturas
	var cristais_em_outras = 0
	if fila.has("B"):
		cristais_em_outras += fila.count("B")
	if deque.has("B"):
		cristais_em_outras += deque.count("B")
	
	if cristais_em_outras > 0:
		return {"is_valid": false, "message": "Existem %d Cristais em outras estruturas. Use os botões de redirecionamento para movê-los para o topo do Túnel Invertido." % cristais_em_outras}
	
	return {"is_valid": false, "message": "Não foram encontrados Cristais (B) suficientes. Você precisa adicionar pelo menos 3 Cristais usando o menu de seleção."}
...
func mostrar_solucao():
	...
	# Configurar a solução correta: 3 Cristais (B) no topo da pilha
	pilha.append("B")  # 1º Cristal (topo)
	pilha.append("B")  # 2º Cristal
	pilha.append("B")  # 3º Cristal
	
	# Adicionar José e outros elementos para contexto
	fila.append("E")  # José na fila
	deque.append("A") # Pedra no deque
	deque.append("F") # Moeda no deque
...
func salvar_resultado():
	...
	if primeira_conclusao:
		...
		if ProgressManager:
			...
			ProgressManager.mark_level_completed("nivel3", pontos)
...
José, após fazer negócios com o vendedor, descobriu que poderia conseguir ótimos negócios, caso ele consiga Cristais, para oferecê-lo.
Ajude o José a ter 3 Cristais no topo do Túnel Invertido.
""" 
...
func _on_btn_fechar():
	...
	save.set_value("nivel3", "historia_vista", true)
