...
func verificar_primeira_conclusao():
	...
	if ProgressManager:
		var level_data = ProgressManager.get_level_data("nivel10", "1")
...
func verificar_solucao() -> Dictionary:
	# Ordens esperadas para cada estrutura
	var ordem_deque_esperada = ["C", "G"]
	var ordem_fila_esperada = ["E", "D", "B"]
	var ordem_pilha_esperada = ["F", "H", "B", "F"]
	
	var erros = []
	
	# Verificar deque (Galeria Dupla)
	if deque != ordem_deque_esperada:
		if deque.size() != ordem_deque_esperada.size():
			erros.append("O deque (Galeria Dupla) não tem a quantidade correta de elementos. Esperados: %d, Atuais: %d." % [ordem_deque_esperada.size(), deque.size()])
		else:
			for i in range(deque.size()):
				if deque[i] != ordem_deque_esperada[i]:
					var elemento_esperado = elementos_caverna[ordem_deque_esperada[i]]["nome"]
					var elemento_atual = elementos_caverna[deque[i]]["nome"] if elementos_caverna.has(deque[i]) else "Desconhecido"
					var posicao = ""
					match i:
						0: posicao = "primeira posição"
						1: posicao = "segunda posição"
					erros.append("Ordem incorreta no deque (%s). Esperado: %s (%s), Atual: %s (%s)." % [posicao, ordem_deque_esperada[i], elemento_esperado, deque[i], elemento_atual])
					break
	
	# Verificar fila (Túnel Linear)
	...
	# Verificar pilha (Túnel Invertido)
	...
	if erros.size() == 0:
		return {"is_valid": true, "message": "🎉 Parabéns! José dominou a bruxaria mais avançada! Todas as três estruturas estão com as ordens corretas!"}
	...
	mensagem_erro += "\nLembre-se das ordens esperadas:\n"
	mensagem_erro += "- Galeria Dupla (deque): Cogumelo, Pérola\n"
	mensagem_erro += "- Túnel Linear (fila): José, Osso, Cristal\n"
	mensagem_erro += "- Túnel Invertido (pilha): Moeda, Diamante, Cristal, Moeda (do topo para o fundo)"
...
func mostrar_solucao():
	...
	# Preencher o deque (Galeria Dupla) com: C, G
	deque.append("C")  # Cogumelo
	deque.append("G")  # Pérola
	
	# Preencher a fila (Túnel Linear) com: E, D, B
	fila.append("E")  # José
	fila.append("D")  # Osso
	fila.append("B")  # Cristal
	
	# Preencher a pilha (Túnel Invertido) com: F, H, B, F
	pilha.append("F")  # Topo - primeira Moeda
	pilha.append("H")  # Diamante
	pilha.append("B")  # Cristal
	pilha.append("F")  # Fundo - segunda Moeda
...
func salvar_resultado():
	...
	if primeira_conclusao:
		...
		if ProgressManager:
			...
			ProgressManager.mark_level_completed("nivel10", pontos)
...
José fará a sua bruxaria mais avançada: dessa vez, ele utilizará os três caminhos.
Em Galeria Dupla, você precisará de: Cogumelo, Pérola
Em Túnel Linear, você precisará de: José, Osso, Cristal
Em Túnel Invertido, você precisará de: Moeda, Diamante, Cristal, Moeda
""" 
...
func _on_btn_fechar():
	...
	save.set_value("nivel10", "historia_vista", true)
