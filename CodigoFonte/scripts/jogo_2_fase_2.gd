...
func configurar_texto_tutorial():
	...
	[b]Dica:[/b] Comece pela placa raiz (2) e depois adicione os filhos 1 (esquerda) e 3 (direita).
	A ordem pré-ordem (RAIZ-ESQUERDA-DIREITA) deve resultar em: 2, 1, 3
	"""
...
func verificar_primeira_conclusao():
	...
	if err == OK and save.has_section_key("arvore_binaria_nivel2", "concluido"):
...
func _on_submit_pressed():
	...
	var validation_result = validate_level_2_ra()
...
func mostrar_solucao():
	print("🎯 Aplicando solução do nível 2 (Rá)...")
	...
	# Adicionar nós na ordem correta: 2 (raiz), 1 (esquerda), 3 (direita)
	...
	if nodes.size() >= 3:
		...
		print("✅ Solução Rá aplicada: Árvore 2-1-3")
		
		if mensagem_label:
			mensagem_label.text = "Solução aplicada! Árvore de Rá: 2 (raiz), 1 (esquerda), 3 (direita)"
	else:
...
func validate_level_2_ra() -> Dictionary:
	if nodes.size() != 3:
	...
	if root_value != "2":
		return {"is_valid": false, "message": "Rá ordena que a raiz seja 2! Sua raiz é " + root_value + "."}
	...
	if preorder != expected_preorder:
		return {"is_valid": false, "message": "A ordem de Rá não foi seguida! Pré-ordem deve ser: 2, 1, 3"}
...
func salvar_resultado():
	...
	save.set_value("arvore_binaria_nivel2", "concluido", true)
	save.set_value("arvore_binaria_nivel2", "pontuacao", pontuacao)
	save.set_value("arvore_binaria_nivel2", "tempo", tempo_decorrido)
	...
	if ProgressManager:
		...
		ProgressManager.mark_level_completed("arvore_binaria_nivel2", pontuacao)
